...
export function AboutUsMissionPanel() {
  return (
    <motion.section
      ...
      className="relative overflow-x-hidden bg-[#f9f9f9] md:py-20 py-10  pb-10 md:pb-20"
      variants={fadeIn}
      initial="hidden"
      whileInView="visible"
    ...
    >
      <div className="mx-auto w-full max-w-[1440px]" style={{ paddingInline: `clamp(1.5rem, 6vw, ${landingTokens.gutter}px)` }}>
        <motion.div
          className="flex w-full flex-col items-start rounded-[20px] border !border-[#D4D4D499] bg-white"
        ...
        >
          <div className="flex w-full flex-col items-start" style={{ gap: fluid(16, 30) }}>
            ...
            <p
              className="m-0 w-full font-normal text-[rgba(0,0,0,0.8)] [font-family:Jost,sans-serif]"
            ...
            <p
              className="m-0 w-full font-semibold text-[rgba(0,0,0,0.8)] [font-family:Jost,sans-serif]"
              style={{ fontSize: BODY_SIZE, lineHeight: 1.6 }}
            >
          ...
          <div className="grid w-full grid-cols-1 md:grid-cols-2" style={{ gap: fluid(16, 24) }}>
            {MISSION_CARDS.map((card) => (
              <div
                key={card.id}
                className="flex w-full flex-col items-start rounded-[20px] border !border-[#D4D4D499] bg-[#f9f9f9]"
